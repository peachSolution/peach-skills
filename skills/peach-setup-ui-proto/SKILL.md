...
description: |
  Frontend-Only UI Proto 프로젝트에 피치 하네스 시스템을 설정합니다. CLAUDE.md를 최소 진입점으로 정리하고, AGENTS.md를 70~90줄 규모로 생성/업데이트합니다.
...
| 섹션 | 소스 파일 |
|------|---------|
| 1. 공통 원칙 | `peach-setup-ui-proto/references/01-common.md` |
| 2. 프론트엔드 규칙 | `peach-setup-ui-proto/references/02-frontend.md` |
| 3. 테스트 및 품질 | `peach-setup-ui-proto/references/03-testing.md` |
| 4. Bounded Autonomy | `peach-setup-ui-proto/references/04-bounded-autonomy.md` |
...
| 카테고리 | 내용 |
|---------|------|
| 금지사항 | `_common`만 import, 옵셔널/null/undefined 금지, isLoading 금지 |
...
| 포인터 | 가이드 코드 경로, 품질 검증 명령어, 공통 컴포넌트 경로(`_common/components/`) |
